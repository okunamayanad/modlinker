...
function addModrinthButton(
  doesModrinthExist,
  projectName,
  result,
  doesModrinthThrowError,
  isCurseforge
) {
  let modrinthLink;
  ...
  buttonWrapper.setAttribute(
    "style",
    "transition: all 0.5s cubic-bezier(0.16, 1, 0.3, 1); !important; display: flex;vertical-align: middle; height:0; justify-content: flex-end; transform: translateY(0px); opacity: 1; gap: 10px;"
  );
  buttonWrapper.classList.add("loading-anim-modrinth");
  ...
  modrinthButton.setAttribute(
    "style",
    "max-width: 120px; padding: 5px; border-radius: 5px; width: auto;height: 20px;display: block; transform: translateY(-64px);"
  );

  let modrinthSvg = document.createElementNS(
  ...
  let modrinthPath = document.createElementNS(
    "http://www.w3.org/2000/svg",
    "path"
  );
  modrinthPath.setAttribute("fill-rule", "nonzero");
  modrinthPath.setAttribute(
    "d",
  ...
  let modrinthPath2 = document.createElementNS(
    "http://www.w3.org/2000/svg",
    "path"
  );
  modrinthPath2.setAttribute(
    "d",
  ...
  let modrinthPath3 = document.createElementNS(
    "http://www.w3.org/2000/svg",
    "path"
  );
  modrinthPath3.setAttribute(
    "d",
  ...
  let modrinthPath4 = document.createElementNS(
    "http://www.w3.org/2000/svg",
    "path"
  );
  modrinthPath4.setAttribute(
    "d",
  ...
  buttonWrapper.appendChild(modrinthButton);
  ...
  function forwardRequest(message) {
    return new Promise((resolve, reject) => {
      chrome.runtime.sendMessage(
        ["checkIfExists", message, new XMLHttpRequest()],
        (response) => {
  ...
  for (var i = 0; i < results.length; i++) {
    ...
    if (href?.includes("curseforge") && href?.includes("mc-mods")) {
      ...
      if (linkArgs[2].includes("search")) {
        ...
        projectName = linkArgs[2].split("&")[3].split("=")[1];
      } else {
      ...
      var doesModrinthExist = await forwardRequest(
        "https://api.modrinth.com/v2/project/" + projectName
      ).catch((err) => {
        console.log("error", err);
        if (err.statusCode !== 404) {
          doesModrinthThrowError = True;
        }
        return false;
      });
      let aContainer = addModrinthButton(
        doesModrinthExist,
        projectName,
        result,
        doesModrinthThrowError,
      ...
      href?.includes("9minecraft") &&
      !href?.includes("shader") &&
      (href?.includes("-mod/") || href?.includes("-api/"))
    ) {
      ...
      href?.includes("planetminecraft") &&
      (href?.includes("/mod/") || href?.includes("/data-pack/"))
    ) {
